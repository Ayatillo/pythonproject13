...
async def play(websocket, game, player, connected):
    ...
    async for message in websocket:
        ...
        if move_counter >= max_moves:  # Проверяем, достигнуто ли максимальное количество ходов
            event = {
                "type": "draw",  # Добавляем сообщение о ничьей
            }
            websockets.broadcast(connected, json.dumps(event))
            break  # Выходим из цикла после ничьей

        if game.winner is not None:
            break  # Выходим из цикла, если есть победитель

async def start(websocket):
    game = Connect4()
    connected = {websocket}

    join_key = secrets.token_urlsafe(13)
